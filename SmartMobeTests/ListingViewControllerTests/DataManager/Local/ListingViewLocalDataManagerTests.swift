...
class ListingViewLocalDataManagerTests: XCTestCase {
    ...
    func test_saveListItem_andReturnList_ifSuccessfullySaved(){
        let localDataManager = ListingViewLocalDataManager()
        try! localDataManager.saveListItem(id: 1, url: "URL", largeUrl: "LargeURL", sourceId: "SourceID")
        ...
        XCTAssertEqual( dataOne?.sourceId, "SourceID")
    }
    
    func test_saveListItems_andReturnLists_ifSuccessfullySaved(){
        let localDataManager = ListingViewLocalDataManager()
        
        let dummyData: [[String]] = [ [ "1","URL", "LargeURL", "SourceID"], [ "2","URL2", "LargeURL2", "SourceID2"], ["3","URL3", "LargeURL3", "SourceID3"]]
        
        for data in dummyData{
            try! localDataManager.saveListItem(id: Int(data[0]) as? Int, url: data[1] as? String, largeUrl: data[2] as? String, sourceId: data[3] as? String)
        }
        ...
        for index in 0..<listOfData.count{
            ...
            XCTAssertEqual( listOfData[index].sourceId, dummyData[index][3])
        }
    }
